...
buildings <- geojson_sf("~/data/Building_Footprints.geojson") %>%
     st_transform(., crs = 2263)

# BLM protests
blm_csv <- read.csv(here("~/data/blmprotestpts.csv"))
...
bm_nyc_cen <- stack("~/data/bm_nyc_cen.grd")
...
polo_frame <- raster::extract(x = polo_s, 
                              y = st_transform(polo, st_crs(polo_s)), 
                              exact = TRUE, na.rm = TRUE) %>% unlist(.) %>% 
  as.data.frame(.) %>% 
