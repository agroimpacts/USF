...
nytract <- st_read(here("nyct2020.shp"))
...
nycha <- geojson_sf(here("nycha.geojson")) %>% 
   st_transform(ny_crs)
...
ny_pop <- read.csv(here("nyct2020_pop.csv"))
...
buildings <- st_read(here("buildings_p.shp")) %>% st_transform(ny_crs)
...
#knitr::include_graphics("C:/CLARK/GEOSPAAR/floodlightblm/ph_overview.png")
```
...
```{r, eval = FALSE}
blm_csv <- read.csv(here("blmprotestpts.csv"))

s3 <- stack(here("bm_nyc.grd"))
...
blm_csv_night <- read.csv(here("blmprotestpts.csv"))
...
```{r, out.width = "60%", echo = FALSE, fig.align = 'center'}
#knitr::include_graphics("C:/CLARK/GEOSPAAR/floodlightblm/mean_lights.png")
```
...
print(s2)
#Query Formatting
instal_sub <- subset(s2, which(getZ(s2) >= '2015-7-01' & (getZ(s2) <= '2016-03-10')))
...
after_inst_sub <- subset(s2, which(getZ(s2) >= '2016-03-10' & (getZ(s2) <= '2017-03-10')))
...
sub1 <- subset(s2, which(getZ(s2) >= '2014-07-01' & (getZ(s2) <= '2017-03-10')))
polo_s <- crop(x = sub1, y = polo) # error no non-missing arguments to min
polo_frame <- raster::extract(x = polo_s, y = polo, exact = TRUE, na.rm = TRUE) %>% unlist(.) %>% 
  as.data.frame(.) %>% 
...
#knitr::include_graphics("graphics\\polo_table.png")
```
...
```{r, echo = FALSE, out.width = "49%", out.height = "49%"}
